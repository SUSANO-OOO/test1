...
if(openingCanvas&&!reduced&&!skipIntro){
  let octx,ow,oh,ot=0;
  const sparks=Array.from({length:innerWidth<700?20:innerWidth<1100?30:40},()=>({a:(Math.random()-.5)*.5,s:2.1+Math.random()*5.9,l:18+Math.random()*58,d:.65+Math.random()*.35,p:Math.random(),side:Math.random()<.5?-1:1}));
...
function drawAmbient(){if(document.hidden){setTimeout(drawAmbient,350);return}actx.clearRect(0,0,aw,ah);stars.forEach(st=>{st.y-=st.v;st.p+=.012;if(st.y<-4){st.y=ah+4;st.x=Math.random()*aw}const a=.045+(Math.sin(st.p)+1)*.025;actx.fillStyle=`rgba(255,255,255,${a})`;actx.fillRect(st.x,st.y,st.s,st.s)});setTimeout(drawAmbient,50)}
resizeAmbient();addEventListener('resize',resizeAmbient,{passive:true});if(reduced||skipIntro)drawAmbient();else setTimeout(drawAmbient,2650);
