...
if(openingCanvas&&!reduced&&!skipIntro){
  let octx,ow,oh,ot=0;
  const sparks=Array.from({length:innerWidth<700?28:innerWidth<1100?42:64},()=>({a:(Math.random()-.5)*.5,s:2.1+Math.random()*5.9,l:18+Math.random()*58,d:.65+Math.random()*.35,p:Math.random(),side:Math.random()<.5?-1:1}));
